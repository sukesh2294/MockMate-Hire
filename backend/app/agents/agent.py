...
async def my_agent(ctx: JobContext):
    # 1. Sabse pehle WebRTC room me connect karein (Critical for Audio)
    print(f"Connecting worker to room: {ctx.room.name}")
    await ctx.connect(auto_subscribe=AutoSubscribe.AUDIO_ONLY)
    print(f"WebRTC connected successfully to: {ctx.room.name}")
        ...
        async with async_session() as session_db:
            ...
            if sess:
                ...
                if is_practice:
                    ...
                    q_stmt = (
                        select(Question)
                        .where(Question.interview_id == sess.interview_id)
    ...
    stt_plugin = (
        ...
        if os.getenv("DEEPGRAM_API_KEY")
        else inference.STT(model="deepgram/nova-3")
    )
    ...
    print(f"AgentSession started for room {room_name}; speaking initial greeting.")

    # 7. Initial greeting trigger
    try:
        await session.generate_reply(
            instructions="Welcome the candidate warmly to MockMate, introduce yourself as the AI interviewer, and ask them to introduce themselves."
        )
        print(f"Initial speech synthesized successfully for room {room_name}.")
    except Exception as exc:
        print(f"Initial reply failed for room {room_name}: {exc!r}")
...
if __name__ == "__main__":
    cli.run_app(
        WorkerOptions(
            ...
            load_threshold=2.5,        # 2.5 load threshold taaki initial spike par worker disconnect na ho
            prewarm_fnc=None,
        )
    )