...
async def create_livekit_session(
    ...
    if payload.mode == "practice":
        if not payload.practice_session_id:
            raise HTTPException(status_code=400, detail="practice_session_id is required for practice rooms")
        full_room_name = f"practice-{payload.practice_session_id}"
    else:
        if not payload.interview_id:
            raise HTTPException(status_code=400, detail="interview_id is required for interview rooms")
